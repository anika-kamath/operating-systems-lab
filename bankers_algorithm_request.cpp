...
int main() { 
    ...
    //process requests for resource
    int pnum,f,add_resource[m];
    cout<<"\nIs any process requesting for resource? Enter 1 for Yes and 0 for No: ";
    cin>>f;

    if(f==1){
        cout<<"Enter the process number which is requesting for resource (Assuming first process is P0): ";
        cin>>pnum;
        cout<<endl<<"Enter the additional resource required (A,B,C,D): ";
        for(int i=0;i<m;i++){
            cin>>add_resource[i];
            allocation[pnum][i]+=add_resource[i];
        }
    }
